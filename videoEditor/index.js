	...
	project = {
		assets: [
			...
			},
			{
				...
				id: 123456,
				rawFile: File,
				frameArray: getFramesFromVideoTrackAsBitmapArray,
			},
		],
		elements: [
...
$('#assetsWin .addFilesButton').addEventListener('click', async () => {
	...
	// importVideoToAssets(fileCache, await getFramesFromVideoTrackAsBitmapArray(await getVideoTrack(await getFileContentsAsDataURL(fileCache))));
	
	await importImageToAssets(fileCache);
...
File.prototype.getContentsAsText = function(format){
	return getFileContentsAsText(this,format);
}
...
File.prototype.getContentsAsDataURL = function(){
	return getFileContentsAsDataURL(this);
}
...
function getFramesFromVideoTrackAsBitmapArray(src){
	let promise = new Promise((resolve, reject) => {
